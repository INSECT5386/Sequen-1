...
if not os.path.exists(DATA_PATH):
    download_file(
        "https://huggingface.co/datasets/Yuchan5386/Kodex-data/resolve/main/output.jsonl?download=true",
        DATA_PATH
    )

if not os.path.exists(TOKENIZER_PATH):
    download_file(
        "https://huggingface.co/datasets/Yuchan5386/Kodex-data/resolve/main/en_bpe.model?download=true",
        TOKENIZER_PATH
    )
...
def jsonl_stream(file_path, limit=None, sample_rate=1.0):
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            # <start> ... <end> 포맷으로 감싸기
            full = f"<start> {text} <end>"

            input_ids = text_to_ids(full)
            target_ids = text_to_ids(text + " <end>")
...
dataset = tf.data.Dataset.from_generator(
    lambda: jsonl_stream(DATA_PATH, limit=78264),
    output_signature=(
...
with strategy.scope():
    ...
    history = model.fit(dist_dataset, epochs=5, verbose=1)

# =======================
# 가중치 저장
# =======================
model.save_weights("tf_model_gpu_safe.weights.h5")
print("✅ 모델 가중치 저장 완료!")
